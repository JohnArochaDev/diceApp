...
export const D4 = () => {
  ...
  const { scene: d4 } = useGLTF("/models/D4/mesh/d4.glb");
  const { scene: d4Collider } = useGLTF("/models/D4/mesh/d4Collider.glb");

  const geometry =
    d4Collider.children[0] instanceof THREE.Mesh
      ? d4Collider.children[0].geometry
      : null;
  if (!geometry) throw new Error("No valid geometry found in d4Collider.glb");
