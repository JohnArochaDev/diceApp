...
import * as BufferGeometryUtils from "three/addons/utils/BufferGeometryUtils.js"; // Adjust path if needed
...
export const D10 = () => {
  ...
  // Helper to merge all mesh geometries in the object, this one is nested
  function mergeGeometriesFromObject(
    ...
    merged.computeVertexNormals(); // Optional: For better lighting/physics if needed
    return merged;
  }

  // In your component:
  const { scene: d10 } = useGLTF("/models/D10/mesh/d10.glb");
  const { scene: d10Collider } = useGLTF("/models/D10/mesh/d10Collider.glb");

  const geometry = mergeGeometriesFromObject(d10Collider);
  if (!geometry) throw new Error("No valid geometry found in d10Collider.glb");
