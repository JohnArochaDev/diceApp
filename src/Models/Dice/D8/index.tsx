...
export const D8 = () => {
  ...
  const { scene: d8 } = useGLTF("/models/D8/mesh/d8.glb");
  const { scene: d8Collider } = useGLTF("/models/D8/mesh/d8Collider.glb");

  const geometry =
    d8Collider.children[0] instanceof THREE.Mesh
      ? d8Collider.children[0].geometry
      : null;
  if (!geometry) throw new Error("No valid geometry found in d8Collider.glb");
